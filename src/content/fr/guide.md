...
Sur la page du dossier, la mise en page est organisée de haut en bas pour aller de l'essentiel au détaillé :

- Le **titre du dossier** est affiché en haut de page sur un bandeau identifié « Dossier LoiClair ».
- Le **résumé IA**, signalé par une icône ✨ et le label « Résumé IA », est présenté dans une carte avec une bordure lumineuse. Il est structuré en **3 sections** : *Ce que dit ce texte* (2-3 phrases simples en langage courant), *Ce qui change concrètement* (3-5 points d'action avec des pastilles teal), et *À retenir* (une seule phrase-clé mise en valeur dans un encart coloré). L'IA tutoie le lecteur pour rendre l'explication plus naturelle et accessible.
- En pied de carte, le **texte résumé** est indiqué avec un lien **« Lire le texte »** vers le document officiel et un bouton **« Changer »** pour sélectionner un autre texte. Les textes sont regroupés par étape législative (Assemblée nationale, Sénat, CMP, Promulguée…) et triés par date. Pour les lois promulguées, le menu inclut aussi les **décrets d'application** et la **version consolidée**.
- Un lien **« Discuter de ce texte avec l'IA »** en or animé permet d'ouvrir une discussion approfondie sur le texte via Perplexity, avec le contexte législatif pré-rempli.
- Si le texte sélectionné n'est pas encore publié, un message l'indique clairement et le bouton « Changer » reste accessible pour sélectionner un autre texte.
- Une section **« À propos de cette loi »** présente les métadonnées du dossier sous forme de grille structurée : **statut** (Promulgué, En cours, Rejeté…), **type de procédure** (avec info-bulle explicative), **auteur** (rôle et nom), **groupe politique**, **date de dépôt**, et **liens vers les sources officielles** (Assemblée nationale, Sénat, Légifrance). Un lien vers le **fil d'actu** du dossier est disponible en bas de cette section.
- Une **timeline verticale** retrace le parcours législatif du dossier étape par étape (Dépôt → Assemblée nationale → Sénat → CMP → Conseil constitutionnel → Promulguée…), avec la **date** de chaque étape et la **durée en jours** entre deux étapes. L'étape en cours est signalée par un point animé et la mention « en cours depuis X j » en orange. Les étapes non encore atteintes apparaissent en pointillés. Chaque étape disposant de textes associés comporte un **accordéon** : en cliquant, vous voyez la liste des textes disponibles (texte déposé, texte de commission, texte adopté en séance…) et pouvez les sélectionner pour en obtenir le résumé IA. L'étape **Promulguée** porte une icône ✓ et affiche en vert la **durée totale** du parcours. Après la promulgation, la timeline affiche les **décrets d'application** et le statut d'**application** de la loi.
...
> Pendant la génération, un message **« analyse en cours… »** s'affiche à côté du titre « Résumé IA ». Si la génération échoue, un message d'erreur apparaît avec un bouton **Réessayer →** pour relancer la demande sans recharger la page.
