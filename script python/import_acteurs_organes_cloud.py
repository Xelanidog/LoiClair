...
def importer_acteur(acteur_data, file_name):
    ...
    try:
        ...
        if isinstance(trigramme_raw, dict):
            trigramme = (
                trigramme_raw.get("#text")
                if not trigramme_raw.get("@xsi:nil")
                else None
            )
        else:
        ...
        if isinstance(info_naissance, dict):
            ...
            date_naissance = (
                date_naissance_raw
                if not (
                    isinstance(date_naissance_raw, dict)
                    and date_naissance_raw.get("@xsi:nil")
                )
                else None
            )
            ...
            ville_naissance = (
                ville_naissance_raw
                if not (
                    isinstance(ville_naissance_raw, dict)
                    and ville_naissance_raw.get("@xsi:nil")
                )
                else None
            )
            ...
            dep_naissance = (
                dep_naissance_raw
                if not (
                    isinstance(dep_naissance_raw, dict)
                    and dep_naissance_raw.get("@xsi:nil")
                )
                else None
            )
            ...
            pays_naissance = (
                pays_naissance_raw
                if not (
                    isinstance(pays_naissance_raw, dict)
                    and pays_naissance_raw.get("@xsi:nil")
                )
                else None
            )
        else:
        ...
        if date_naissance:
            try:
                ...
                print(
                    f"Date naissance invalide pour {file_name}: {date_naissance} - Age=None."
                )
        ...
        if isinstance(date_deces_raw, dict):
            date_deces = (
                date_deces_raw.get("#text")
                if not date_deces_raw.get("@xsi:nil")
                else None
            )
        else:
        ...
        if isinstance(profession, dict):
            ...
            libelle_profession = (
                ...
                if not (
                    ...
                    and libelle_profession_raw.get("@xsi:nil")
                )
                else None
            )
            ...
            cat_soc_pro = (
                cat_soc_pro_raw
                if not (
                    isinstance(cat_soc_pro_raw, dict)
                    and cat_soc_pro_raw.get("@xsi:nil")
                )
                else None
            )
            ...
            fam_soc_pro = (
                fam_soc_pro_raw
                if not (
                    isinstance(fam_soc_pro_raw, dict)
                    and fam_soc_pro_raw.get("@xsi:nil")
                )
                else None
            )
        else:
        ...
        adresses = (
            json.dumps(data.get("adresses", {}).get("adresse", []))
            ...
            for mandat in mandats_raw if isinstance(mandats_raw, list) else []:
                if mandat.get("dateFin") is None:  # Actif uniquement
                    ...
                    if type_organe in [
                        ...
                        "GROUPESENAT",
                        "GOUVERNEMENT",
                    ]:  # Priorise GP/GROUPESENAT, puis GOUVERNEMENT
        ...
        try:
                ...
                if mandat.get("dateFin") is None:  # Actif uniquement
                    ...
                    if isinstance(organes, dict):
                        ...
                        if isinstance(organe_ref, str):
                            ...
                            organe_ref, list
                        ):  # Cas liste directe pour organeRef
            ...
            print(
                f"Clé manquante pour mandats dans {file_name}: {ke} - organes_refs=[]"
            )
        organes_refs = (
            list(organes_refs_set) if organes_refs_set else None
        )  # Convertit set en list, ou None si vide
        ...
        try:
            ...
            if isinstance(adresses_section, dict) and adresses_section.get("@xsi:nil"):
                ...
                if isinstance(adresse_raw, dict):
                    ...
                    adresse_list = []
                    print(
                        f"Adresses invalides (ni dict ni list) pour {file_name} - Skip."
                    )
            ...
            for addr in adresse_list:
                if isinstance(addr, dict):
                    ...
                    if addr_type == "15":
                        ...
                        linkedin_url = (
                            f"https://www.linkedin.com{val_elec}" if val_elec else None
                        )
                    elif addr_type == "23":
                        profil_url = val_elec
                    elif addr_type == "24":
                        twitter_url = (
                            f"https://twitter.com/{val_elec.lstrip('@')}"
                            if val_elec
                            else None
                        )
                    elif addr_type in ["26", "29"]:
                        instagram_url = (
                            f"https://instagram.com/{val_elec.lstrip('@')}"
                            if val_elec
                            else None
                        )
                    elif addr_type == "25":
                        facebook_url = (
                            f"https://www.facebook.com/{val_elec}" if val_elec else None
                        )
        ...
        if isinstance(mandats_raw, dict):  # Wrapper si single mandat
            mandats_raw = [mandats_raw]
        for mandat in mandats_raw if isinstance(mandats_raw, list) else []:
            if mandat.get("dateFin") is None and mandat.get("typeOrgane") in [
                "ASSEMBLEE",
                "SENAT",
            ]:
...
def importer_organe(organe_data, file_name):
    ...
    try:
        ...
        try:
            nombre_reunions_annuelles = (
                int(nombre_reunions_annuelles_raw)
                if nombre_reunions_annuelles_raw
                else None
            )
...
def importer_acteurs_organes_from_zip(zip_ref: zipfile.ZipFile):
    ...
    for file_name in tqdm(json_files, desc="Import acteurs/organes", unit="fichier"):
        try:
            ...
            if len(acteurs_batch) >= batch_size:
                response = (
                    supabase.table("acteurs")
                    .upsert(acteurs_batch, on_conflict="uid")
                    .execute()
                )
                if response.data:
                    ...
                    print(
                        f"Erreur batch acteurs : {getattr(response, 'error', 'Erreur inconnue')}"
                    )
            ...
            if len(organes_batch) >= batch_size:
                response = (
                    supabase.table("organes")
                    .upsert(organes_batch, on_conflict="uid")
                    .execute()
                )
                if response.data:
                    ...
                    print(
                        f"Erreur batch organes : {getattr(response, 'error', 'Erreur inconnue')}"
                    )
    ...
    if acteurs_batch:
        response = (
            supabase.table("acteurs").upsert(acteurs_batch, on_conflict="uid").execute()
        )
        if response.data:
            success_acteurs += len(acteurs_batch)
        else:
    ...
    if organes_batch:
        response = (
            supabase.table("organes").upsert(organes_batch, on_conflict="uid").execute()
        )
        if response.data:
            success_organes += len(organes_batch)
        else:
