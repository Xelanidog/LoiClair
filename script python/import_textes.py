...
def importer_texte(file_path):
    ...
    try:
        ...
        if not uid:
            ...
            return
        # Gestion des divisions (tomes) : importer chaque tome comme texte autonome
        divisions = texte.get('divisions', {}).get('division', []) if texte.get('divisions') else []
        if divisions:
            # Skip l'import du parent (wrapper) ; importer seulement les tomes
            ...
            for idx, division in enumerate(divisions):
                ...
                data_tome = {
                    ...
                    'auteurs_refs': auteurs_refs_tome,
                ...
                if response_tome.data:
                    print(f"Tome {uid_tome} importé avec succès depuis {file_path}.")
                else:
