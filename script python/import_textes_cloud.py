        ...
        if 'dossierParlementaire' in data:
            try:
                ...
                if acts_leg is None or not isinstance(acts_leg, dict):
                    ...
                    print(f"Debug: actesLegislatifs absent ou invalide pour {file_name}, set actes=[]")
                else:
                    actes = acts_leg.get('acteLegislatif', [])  # Safe get avec default []
                ...
                for acte in actes:
                    ...
                    if acte.get('codeActe') == 'PROM':
                        ...
                        if sub_acts is None or not isinstance(sub_acts, dict):
                            print(f"Debug: sub_acts absent ou invalide pour PROM dans {file_name}, skip PROM.")
                            continue  # Skip ce PROM invalide
                        prom_acte = sub_acts.get('acteLegislatif')

                        if prom_acte is None:
                            print(f"Debug: prom_acte is None pour PROM dans {file_name}, set to {{}}.")
                            prom_acte = {}
                            ...
                            print(f"Debug: prom_acte inattendu (type={type(prom_acte)}) pour PROM dans {file_name}, set to {{}}.")
...
def importer_textes_from_zip(zip_ref: zipfile.ZipFile):
    ...
    json_files = [f for f in zip_ref.namelist() 
                  if (f.startswith('json/document/') or f.startswith('json/dossierParlementaire/')) 
                  and f.endswith('.json')]
    ...
    for file_name in tqdm(json_files, desc="Import textes", unit="fichier"):
        try:
            ...
            if len(batch) >= batch_size:
                ...
                if not response.data:  # ← Changement : vérifie si pas de data (erreur probable)
                    print(f"Erreur batch : {getattr(response, 'error', 'Erreur inconnue')}")
                    failed += len(batch)  # Ajoute aux échecs
                else:
                    success += len(batch)  # ← Déplace ici : compte succès seulement si OK
                batch = []
    ...
    if batch:
        ...
        if not response.data:
            print(f"Erreur batch final : {getattr(response, 'error', 'Erreur inconnue')}")
            failed += len(batch)
        else:
            success += len(batch)
